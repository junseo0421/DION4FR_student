...
class FAM_KD(nn.Module):
    def __init__(self, in_channels, in_channels_x, out_channels, mid_channel, shapes):
        super(FAM_KD, self).__init__()
        self.shapes = shapes
        self.out_shapes = shapes
    ...
    def forward(self, features_student, features_teacher):
        ...
        for features, abf, shape, out_shape in zip(x[1:], self.atfs[1:], self.shapes[1:], self.out_shapes[1:]):
            out_features, res_features = abf(features, res_features, shape, out_shape)
            results.insert(0, out_features)

        # losses
        loss_fam_kd = self.famkd_loss_weight * feat_loss(results, features_teacher)

        return loss_fam_kd


class CROSSATF(nn.Module):
    def __init__(self, in_channel, in_channel_x, mid_channel, out_channel, fuse, shape, out_shape):
    ...
    def forward(self, x, y=None, shape=None, out_shape=None):
        ...
        if self.conv1_x is not None:
            x = self.conv1_x(x)  # mid channel
            x_residual = x

        if self.att_conv is not None:
            # reduce channel dimension of residual features
            if self.conv1 is not None:
                y = self.conv1(y)  # mid channel
            # upsample residual features
            y = F.interpolate(y, (shape, shape), mode="nearest")
            # fusion
            x = self.att_conv(x, y)  # mid channel
        if x.shape[-1] != out_shape:
            x = F.interpolate(x, (out_shape, out_shape), mode="nearest")
        y = self.conv2(x)  # out channel
