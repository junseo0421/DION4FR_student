    ...
    with tqdm(total=len(train_loader), desc=f"Training Epoch {epoch}") as pbar:
        for batch_idx, (gt, mask_img) in enumerate(train_loader):  # 24.09.20 수정
            ...
            for _ in range(1):
                ...
                ### feature KD loss
                feature_kd_loss = ofd_loss_1(f1_s, f1_t) + ofd_loss_2(f2_s, f2_t) + ofd_loss_3(f3_s, f3_t) + ofd_loss_4(f4_s, f4_t)
...
if __name__ == '__main__':
    ...
    ofd_loss_1 = OFD_CON(in_channels=4, out_channels=32).cuda(0)
    ofd_loss_2 = OFD_CON(in_channels=8, out_channels=64).cuda(0)
    ofd_loss_3 = OFD_CON(in_channels=16, out_channels=128).cuda(0)
    ofd_loss_4 = OFD_CON(in_channels=32, out_channels=256).cuda(0)

    opt_gen = optim.Adam(
        list(gen.parameters()) +
        list(ofd_loss_1.parameters()) +
        list(ofd_loss_2.parameters()) +
        list(ofd_loss_3.parameters()) +
        list(ofd_loss_4.parameters()), lr=args.lr_G, betas=(0.5, 0.999), weight_decay=1e-4)
