...
def train(gen, dis, opt_gen, opt_dis, epoch, train_loader, writer, teacher_gen):  # 24.09.19 recognizer
    ...
    with tqdm(total=len(train_loader), desc=f"Training Epoch {epoch}") as pbar:
        for batch_idx, (gt, mask_img) in enumerate(train_loader):  # 24.09.20 수정
            ...
            features_s_list = [features_s[key] for key in ["x1", "x2", "x3", "x4"]]
            ...
            with torch.no_grad():
                ...
                features_t_list = [features_t[key] for key in ["x1", "x2", "x3", "x4"]]

            original_kd_loss = mae(teacher_pred, I_pred) * 20  # 가중치는 pxiel_rec_loss와 똑같이 설정. 나중에 조절 필요할 수도

            ### feature KD loss
            feature_kd_loss = fam_loss(features_s_list, features_t_list)
            feature_kd_loss_weight = 3.0
...
if __name__ == '__main__':
    NAME_DATASET = 'HKdb-1'
    SAVE_BASE_DIR = '/content/drive/MyDrive/fam_kd/output'
    ...
    in_channels_x = [4, 8, 16, 32]
    out_channels = [4, 8, 16, 32]
    mid_channel = 32
    shapes = [24, 48, 96, 192]

    fam_loss = FAM_KD(in_channels=in_channels, in_channels_x=in_channels_x, out_channels=out_channels,
                      mid_channel=mid_channel, shapes=shapes).cuda(0)
