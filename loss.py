...
class VID_NO_REG(nn.Module):
    def __init__(self, out_channels, init_var=5.0, eps=1e-6):
        super(VID_NO_REG, self).__init__()
        self.eps = eps
        # self.regressor = nn.Sequential(*[
        #     nn.Conv2d(in_channels, mid_channels, kernel_size=1, stride=1, padding=0, bias=False),
        #     # nn.BatchNorm2d(mid_channels),
...
class DCD(nn.Module):
    ...
    def __init__(self, s_dim, t_dim, feat_dim=128, max_tau=10.0, alpha=0.5, init_tau=1.0, init_b=0.0):
...
class CRD(nn.Module):
    ...
    '''
    def __init__(self, s_dim, t_dim, n_data, feat_dim=128, nce_n=4096, nce_t=0.07, nce_mom=0.5):
