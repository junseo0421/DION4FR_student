...
if __name__ == '__main__':
    NAME_DATASET = 'SDdb-2'
    SAVE_BASE_DIR = '/content/drive/MyDrive/srd_kd/output'
    ...
    srd_loss_1 = SRD(s_dim=4, t_dim=4).cuda(0)
    srd_loss_2 = SRD(s_dim=8, t_dim=8).cuda(0)
    srd_loss_3 = SRD(s_dim=16, t_dim=16).cuda(0)
    srd_loss_4 = SRD(s_dim=32, t_dim=32).cuda(0)

    opt_gen = optim.Adam(
        list(gen.parameters()) +
        list(srd_loss_1.parameters()) +
        list(srd_loss_2.parameters()) +
        list(srd_loss_3.parameters()) +
        list(srd_loss_4.parameters()), lr=args.lr / 2, betas=(0.0, 0.9), weight_decay=1e-4
    )
