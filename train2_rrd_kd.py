...
def train(gen, dis, opt_gen, opt_dis, epoch, train_loader, writer, teacher_gen):  # 24.09.19 recognizer
    ...
    with tqdm(total=len(train_loader), desc=f"Training Epoch {epoch}") as pbar:
        for batch_idx, (gt, mask_img) in enumerate(train_loader):  # 24.09.20 수정
            ...
            ### feature KD loss
            feature_kd_loss = rrd_1(f1_s, f1_t) + rrd_2(f2_s, f2_t) + rrd_3(f3_s, f3_t) + rrd_4(f4_s, f4_t)
            feature_kd_loss_weight = 0.1
...
if __name__ == '__main__':
    ...
    rrd_1 = RRD(s_dim=4, t_dim=32).cuda(0)
    rrd_2 = RRD(s_dim=8, t_dim=64).cuda(0)
    rrd_3 = RRD(s_dim=16, t_dim=128).cuda(0)
    rrd_4 = RRD(s_dim=32, t_dim=256).cuda(0)
