...
class Cross_Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_emb: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
        short_cut = x
        kv = self.kv(x).reshape(B, N, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        k, v = kv.unbind(0)
        q = self.q(x_emb).reshape(B, N, 1, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q = q.unbind(0)[0]
...
class SwinTransformer4(nn.Module):
    ...
    def __init__(self, pre_step, img_size=224, patch_size=4, in_chans=3,
        ...
        ## 24.10.31 simple TSP
        self.simple_MRB = SimplifiedTSP(pre_step)

        ## 2023-12-07 residual patch reversal layer
        ...
        ## 2023-11-23  cross-attention
        self.cross_attn_layers = nn.ModuleList()
        ##2024-01-19 convolution for mask adjacent
        self.CONV_FOR_cross_attn_layers_left = nn.ModuleList()
        self.CONV_FOR_cross_attn_layers_right = nn.ModuleList()

        ##2024-03-04  absolute positional encoding for cross attention
        self.pos_embed = nn.Parameter(torch.randn(1, 36, 768) * .02)

        for j_layer in reversed(range(self.num_layers - 1)):
            de_layer = Cross_Attention(dim=int(embed_dim * 2 ** j_layer),
                                       num_heads=num_heads[j_layer],
                                       qkv_bias=qkv_bias,)
            conv_layer_left = nn.Sequential(nn.Conv2d(int(embed_dim * 2 ** j_layer), int(embed_dim * 2 ** j_layer), kernel_size=3, padding=1)
                                          ,LayerNorm(int(embed_dim * 2 ** j_layer))
                                          ,nn.LeakyReLU(0.2)
                                          ,)
            conv_layer_right = nn.Sequential(nn.Conv2d(int(embed_dim * 2 ** j_layer), int(embed_dim * 2 ** j_layer), kernel_size=3, padding=1)
                                            , LayerNorm(int(embed_dim * 2 ** j_layer))
                                            , nn.LeakyReLU(0.2)
                                            , )
            self.CONV_FOR_cross_attn_layers_left.append(conv_layer_left)
            self.CONV_FOR_cross_attn_layers_right.append(conv_layer_right)
            self.cross_attn_layers.append(de_layer)
    ...
    def encode_features(self, x):
        ...
        for layer in self.layers:  # encoder / layer = [0, 1, 2, 3]
        ...
        # x = torch.flatten(x, 1)
        out_encode = out.view(out.size(0), H, W, self.num_features)  # B, H, W, C
    ...
    def decode_features(self, x, shortcut_x):
        ...
        ## 2024-03-04  absolute positional embedding
        x = x + self.pos_embed
        ...
        for layer in self.decoder_layers:
            ...
            ## 2023-11-23  skip connection part
            if i < 3:
                shortcutx = shortcut_x[2 - i].view(B, Wh, Wh, -1)[:, :, :, :]

                # Mask adjacent feature stretching
                if i==0:  # 양쪽 3 pixel씩
                    left_tar = shortcutx[:, :, int(Ww * 0.25) : int(Ww * 0.25)+1, :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    left_tar = torch.cat([left_tar] * 3, dim=3)
                    right_tar = shortcutx[:, :,int(Ww * 0.75)-1 : int(Ww * 0.75), :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    right_tar = torch.cat([right_tar] * 3, dim=3)
                elif i==1:  # 양쪽 6 pixel씩
                    left_tar = shortcutx[:, :, int(Ww * 0.25):int(Ww * 0.25) + 1, :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    left_tar = torch.cat([left_tar] * 6, dim=3)
                    right_tar = shortcutx[:, :, int(Ww * 0.75)-1 : int(Ww * 0.75), :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    right_tar = torch.cat([right_tar] * 6, dim=3)
                elif i==2:  # 양쪽 12 pixel씩
                    left_tar = shortcutx[:, :, int(Ww * 0.25):int(Ww * 0.25) + 1, :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    left_tar = torch.cat([left_tar] * 12, dim=3)
                    right_tar = shortcutx[:, :, int(Ww * 0.75)-1 : int(Ww * 0.75), :].clone().detach().permute(0, 3, 1, 2).contiguous()
                    right_tar = torch.cat([right_tar] * 12, dim=3)

                shortcut_left_input = self.CONV_FOR_cross_attn_layers_left[i](left_tar)
                shortcut_right_input = self.CONV_FOR_cross_attn_layers_right[i](right_tar)

                shortcut_left_input = shortcut_left_input.permute(0, 2, 3, 1)
                shortcut_right_input = shortcut_right_input.permute(0, 2, 3, 1)

                shortcutx = torch.cat((shortcut_left_input, shortcutx[:, :, int(Ww * 0.25):Ww - int(Ww * 0.25), :]),dim=2)
                shortcutx = torch.cat((shortcutx, shortcut_right_input), dim=2)

                shortcutx = shortcutx.view(B, Wh * Ww, -1)
                x = self.cross_attn_layers[i](x, shortcutx)

                x = x.view(B, Wh * Ww, -1)
                i = i + 1
                ## region origin U-transformer
                # x = x.view(B, Wh, Ww, -1)
    ...
    def forward(self, x, only_encode=False):
        B = x.size(0)
        x, shortcut_x, out_encode = self.encode_features(x)  # (2, 36, 768), tensor list:4, (2, 6, 6, 768)
        if only_encode:
            return out_encode

        # 24.10.31 simple TSP module
        x, out_decode = self.simple_MRB(x)

        # # 2024.10.15 fsp를 위한 feature 뽑기  <<< 수정 필요
        # fsp_feature_1 = shortcut_x[2]

        x = x.view(B, 4 + 2 * self.pre_step, 4 + 2 * self.pre_step, self.num_features)  # (2, 6, 6, 768)

        # 2023-11-16 Consider only the left and right regions
        x[:, :, 1:5, :] = shortcut_x[3].view(B, 4 + 2 * self.pre_step, 4 + 2 * self.pre_step, self.num_features)[:, :,  # (2, 6, 6, 768)
                          1:5, :]

        x = x.view(B, -1, self.num_features)  # (2, 36, 768)
        out = self.decode_features(x, shortcut_x)

        return out, out_decode
        # return out, out_encode # TSP module이 없어지면 out_encode 사용

## 2023-12-07  residual blocks for patch reversal layer
class ResnetBlock(nn.Module):
    def __init__(self, dim,   activation=nn.ReLU(True), use_dropout=False):
        super(ResnetBlock, self).__init__()
        self.conv_block = self.build_conv_block(dim, 1, 3, activation, use_dropout)

    def build_conv_block(self, dim, padding_size, kernel_size, activation, use_dropout):
        conv_block = []

        conv_block += [nn.Conv2d(dim, dim, kernel_size=kernel_size, padding=padding_size),
                       activation]
        if use_dropout:
            conv_block += [nn.Dropout(0.5)]

        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=padding_size)]
