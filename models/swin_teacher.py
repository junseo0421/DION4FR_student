...
class SwinTransformer4(nn.Module):
    ...
    def encode_features(self, x):
        ...
        for layer in self.layers:  # encoder
    ...
    def forward(self, x, only_encode=False):
        ...
        x, shortcut_x, out_encode = self.encode_features(x) # x : B L C / out_encode : B H W C
        fsp_feature1 = out_encode.permute(0, 3, 1, 2)  # B C H W

        if only_encode:
            return out_encode

        x, out_decode = self.MRB(x)
        fsp_feature2 = out_decode.permute(0, 3, 1, 2)  # B C H W
        fsp_matrix = compute_fsp_matrix(fsp_feature1, fsp_feature2)

        # x = x.view(B, 8, 8, self.num_features)
        x = x.view(B, 4 + 2 * self.pre_step, 4 + 2 * self.pre_step, self.num_features)

        # 2023-11-16 Consider only the left and right regions
        x[:, :, 1:5, :] = shortcut_x[3].view(B, 4 + 2 * self.pre_step, 4 + 2 * self.pre_step, self.num_features)[:, :,
                          1:5, :]
        ...
        return out, out_decode, fsp_matrix
